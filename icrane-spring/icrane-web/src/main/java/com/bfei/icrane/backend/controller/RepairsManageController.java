...
@Controller
@RequestMapping("/dollRepairsManage")
public class RepairsManageController {
	private static final Logger logger = LoggerFactory.getLogger(UserController.class);

	@Autowired
	private DollRepairsService dollRepairsService;
	...
	public String list(String MemberId,@RequestParam(defaultValue="1")Integer page,@RequestParam(defaultValue="50")Integer pageSize,HttpServletRequest request)throws Exception {
		logger.info("查询用户报修输入参数:name={},page={},pageSize={}",MemberId,page,pageSize);
		try {
			if("".equals(MemberId)){
				MemberId=null;
			}
			PageBean<DollRepairsAll> pageBean = dollRepairsService.getDollRepairsList(page, pageSize);
			logger.info("查询用户报修列表结果:{}",pageBean.getList().toString());
			request.setAttribute("pageBean", pageBean);
			return "repairs/repairs_list";
		} catch (Exception e) {
			logger.error("查询用户报修列表SystemError:"+e);
			throw e;
		}
	}
