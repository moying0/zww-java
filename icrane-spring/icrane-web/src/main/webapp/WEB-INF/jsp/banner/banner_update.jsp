...
<nav class="breadcrumb"><i class="Hui-iconfont">&#xe67f;</i> 首页 <span class="c-gray en">&gt;</span> 广告管理<span class="c-gray en">&gt;</span> 修改广告 <a class="btn btn-success radius r" style="line-height:1.6em;margin-top:3px" href="javascript:location.replace(location.href);" title="刷新" ><i class="Hui-iconfont">&#xe68f;</i></a></nav>
	   ...
	   <%--<div class="row cl">
		   ...
		   <div class="row cl">
			...
			<div class="formControls col-xs-8 col-sm-8">
				<input type="text" class="input-text" value="${banner.description}" placeholder="" id="description" name="description">
			</div>
		</div>
		<div class="row cl">
			...
			<div class="formControls col-xs-8 col-sm-8">
				<select class="input-text" id="type" name="type" style="width:200px">
					<option value="0" <c:if test="${banner.type=='0'}">selected="selected"</c:if>>banner</option>
					<option value="1" <c:if test="${banner.type=='1'}">selected="selected"</c:if>>悬浮窗</option>
				</select>
			</div>
		</div>
		<div class="row cl">
			...
			<div class="formControls col-xs-8 col-sm-8">
				<input type="text" class="input-text" value="${banner.hyperlink}" placeholder="" id="hyperlink" name="hyperlink">
			</div>
		</div>
		   <div class="row cl">
			   ...
			   <div class="formControls col-xs-8 col-sm-8">
				   <select class="input-text" id="linkType" name="linkType" style="width:200px">
					   <option value="0" <c:if test="${banner.linkType ==0}">selected="selected"</c:if>>跳转链接</option>
					   <option value="1" <c:if test="${banner.linkType ==1}">selected="selected"</c:if>>跳转支付页</option>
					   <option value="2" <c:if test="${banner.linkType ==2}">selected="selected"</c:if>>跳转分享</option>
					   <option value="3" <c:if test="${banner.linkType ==3}">selected="selected"</c:if>>跳转支付套餐</option>
					   <option value="4" <c:if test="${banner.linkType ==4}">selected="selected"</c:if>>身份验证</option>
				   </select>
			   </div>
		   </div>
		   <div class="row cl">
			   <label class="form-label col-xs-4 col-sm-3"><span class="c-red">*</span>支付第几个套餐：</label>
			   <div class="formControls col-xs-8 col-sm-8">
				   <select class="input-text" id="payIndex" name="payIndex" style="width:200px">
					   ...
					   <c:forEach items="${chargeRules}" var="chargeRules">
						   <c:choose>
							   <c:when test="${chargeRules.id==banner.payIndex}">
		   ...
		   <div class="row cl">
			   ...
			   <div class="formControls col-xs-8 col-sm-8">
				   <input type="text" class="input-text" value="${banner.sorts}" placeholder="例：1" id="sorts" name="sorts">
			   </div>
		   </div>
		 <div class="row cl">
			...
			<input id="validStartDate" name="validStartDate" class="input-text Wdate"
          		onfocus="WdatePicker({dateFmt:'yyyy-MM-dd HH:mm:ss',readOnly:true})"
				   value="<fmt:formatDate value='${banner.validStartDate }' pattern='yyyy-MM-dd HH:mm:ss'/>" />
			</div>	
		</div>
		<div class="row cl">
			<label class="form-label col-xs-4 col-sm-3"><span class="c-red">*</span>过期结束时间：</label>
			<div class="formControls col-xs-8 col-sm-4">
				<input type="text" class="input-text Wdate" placeholder="" id="validEndDate" name="validEndDate"
					   onfocus="WdatePicker({dateFmt:'yyyy-MM-dd HH:mm:ss',readOnly:true})"
					   value="<fmt:formatDate value='${banner.validEndDate }' pattern='yyyy-MM-dd HH:mm:ss'/>" />
			</div>
		</div>
		<div class="row cl">
