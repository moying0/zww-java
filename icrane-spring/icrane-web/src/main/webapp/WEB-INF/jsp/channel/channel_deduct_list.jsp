...
<nav class="breadcrumb"><i class="Hui-iconfont">&#xe67f;</i> 首页 <span class="c-gray en">&gt;</span> 渠道管理<span class="c-gray en">&gt;</span> 渠道扣量列表 <a class="btn btn-success radius r" style="line-height:1.6em;margin-top:3px" href="${path}/channelManage/deductList" title="刷新" ><i class="Hui-iconfont">&#xe68f;</i></a></nav>
<form id="frm" action="channelManage/deductList" method="post" >
	...
	<div class="text-c"> 
	    <span>
			...
			<input type="text" name="registerDate" id="registerDate"
				   value="${registerDate}"
				   onfocus="WdatePicker({dateFmt:'yyyy-MM-dd HH:mm:ss',readOnly:true})" placeholder="输入开始时间" style="width:150px"  class="input-text Wdate">
			<input type="text" name="endDate" id="endDate"
				   value="${endDate}"
				   onfocus="WdatePicker({dateFmt:'yyyy-MM-dd HH:mm:ss',readOnly:true})" placeholder="输入结束时间" style="width:150px"  class="input-text Wdate">
			<select class="input-text" id="lastLoginFrom" name="lastLoginFrom" style="width:150px">
		...
		<table class="table table-border table-bordered table-bg table-hover table-sort table-responsive">
			...
			<tbody>
				<c:forEach items="${pageBean.list}" var="member"> 
					<tr class="text-c">
						<td>${member.loginChannel }</td>
						<td>${member.registerChannel }</td>
						<td>${member.id}</td>
						<td>${member.userId}</td>
						<td>${member.name }</td>
						<td>${member.mobile}</td>
						<td>${member.gender }</td>
						<td>${member.phoneModel }</td>
						...
						<td>
							<a class="btn btn-primary radius" data-title="用户抓取记录查看" data-href="javascript:;" onclick="charge_query('用户抓取记录','channelManage/listCatch','${member.userId}','','')" href="javascript:;">查看</a>
						</td>
						<td>
							<a class="btn btn-primary radius" data-title="用户充值记录查看" data-href="javascript:;" onclick="charge_query('用户充值记录','channelManage/listCharge','${member.userId}','','')" href="javascript:;">查看</a>
						</td>
						<td class="f-14 td-manage">
								 <a style="text-decoration:none" class="ml-5" onClick="member_del(this,'${member.id}')" href="javascript:;" title="删除"><i class="Hui-iconfont">&#xe6e2;</i></a>
						</td>
					</tr>
				</c:forEach>
			</tbody>
			<tfoot>
			    <tr>
					...
					<jsp:param name= "page_url" value= "channelManage/deductList"/>
					</jsp:include>
					</td>
			    </tr>
			</tfoot>
